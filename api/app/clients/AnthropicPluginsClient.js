...
class AnthropicPluginsClient extends AnthropicClient {
  ...
  setOptions(options) {
    ...
    this.agentisLowEndModel = this.agentOptions?.model?.includes('haiku');

    super.setOptions(options);

    this.isLowEndModel = this.modelOptions?.model?.includes('haiku');
  ...
  async initialize({ user, message, onAgentAction, onChainEnd, signal }) {
    ...
    if (this.tools.length > 0 && !this.functionsAgent) {
      this.tools.push(new SelfReflectionTool({ message, isLowEndModel: false }));
    ...
    const handleAction = (action, runId, callback = null) => {
      ...
      logger.debug('Latest Agent Action ', this.actions[this.actions.length - 1]);
  ...
  async executorCall(message, { signal, stream, onToolStart, onToolEnd }) {
    ...
    for (let attempts = 1; attempts <= maxAttempts; attempts++) {
      ...
      logger.debug(`Attempt ${attempts} of ${maxAttempts}`);
      ...
      try {
        ...
        logger.error('executorCall error:', err);
  ...
  async handleResponseMessage(responseMessage, saveOptions, user) {
    ...
    logger.debug('[handleResponseMessage] Output:', {
      output,
      errorMessage,
      ...result,
    });
  ...
  async buildCompletionPrompt({ messages, promptPrefix: _promptPrefix }) {
    ...
    if (!this.isLowEndModel && this.options.reverseProxyUrl) {
      instructionsPayload.role = 'user';
    }
    ...
    if (this.isLowEndModel && messagePayload.content.length > 0) {
    ...
    if (this.functionsAgent && !this.isLowEndModel) {
...
module.exports = AnthropicPluginsClient;
